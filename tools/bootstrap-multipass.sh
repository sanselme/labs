...
for i in {1..3}; do
  multipass transfer "${SSH_PUB_KEY_FILE}" "${MULTIPASS_NAME_PREFIX}-0${i}":authorized_keys
  multipass transfer /tmp/config.sh "${MULTIPASS_NAME_PREFIX}-0${i}":config.sh
  multipass exec "${MULTIPASS_NAME_PREFIX}-0${i}" -- bash config.sh
